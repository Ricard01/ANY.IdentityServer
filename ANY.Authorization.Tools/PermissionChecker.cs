...
public static class PermissionChecker
{
    ...
    public static bool ComplyWithPermissions(this IEnumerable<Permissions> usersPermissions,
    ...
    private static Permissions EnumPermission(string permission)
    {
        // True if the permissionName (attributeName in Controller)  was converted successfully;
        if (!Enum.TryParse(permission, true, out Permissions permissionToCheck))
            throw new InvalidEnumArgumentException($"{permission} could not be converted to a {nameof(Permissions)}.");
